  ...
  return (
    <div className="flex flex-col items-center w-full">
      ...
      <div className="flex w-full shadow-md justify-between">
        <div className="p-5 text-xl font-bold self-start">Listado Completo de Habitaciones</div>
        <div className="flex">
        ...
        <table className="text-sm text-left rtl:text-right text-gray-700">
          ...
          <thead className="text-xs text-white uppercase bg-[#003A70] sticky top-0 z-1 h-8">
            <tr className="place-content-center border-solid border-2">
              <th className="px-3 py-2 border-solid border-2">  </th>
          ...
          <tbody >
            ...
            {allRooms.map((item) => (
              ...
              >
                <td className="px-3 py-2 hover:scale-125 transition-all ease-in-out">
                  {item.photos && item.photos.length > 0 && <img src={item.photos[0]} alt="Room" className="w-12 h-12 object-cover" />}
                </td>
                <td className="px-3 py-2"> <BdRooms roomNum={item.roomNum} /> </td>
                <td className="px-3 py-2"> <BdRooms title={item.title} /> </td>
                <td className="px-3 py-2"> <BdRooms roomType={item.roomType} /> </td>
                <td className="px-3 py-2"> <BdRooms rate={`€ ` + item.rate} /> </td>
                <td className="px-3 py-2"> <BdRooms maxPeople={item.maxPeople} /> </td>
                <td className="px-3 py-2"> <BdRooms status={item.status} /> </td>
                <td className="px-3 py-2"> <BdRooms bedNum={item.bedNum} /> </td>
